...
def export_to_pdf(figures, outputname):
    ...
    images = [base64.b64encode(figure.to_image(format="png")).decode(
        'utf-8') for figure in figures]
...
def append_figure(figure):
    image = base64.b64encode(figure.to_image(format="png")).decode('utf-8')
    _ = template
    _ = _.format(image=image, caption='', width=width, height=height)
    report_html += _


def append_html(text):
    report_html += text


def finish_report(outputname):
    if not os.path.exists("images"):
        os.mkdir("images")
